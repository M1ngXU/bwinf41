...
fn pfad_sortieren(orte: Vec<Ort>, kanten: HashMap<Kante, Vec<Kante>>) -> Option<Wrapper> {
    let max_distance = orte
        .iter()
        .cartesian_product(&orte)
        .map(|(a, b)| a.kosten_zu(b))
        .max()
        .unwrap();
...
impl Pfad2 {
    fn new(orte: Vec<Ort>, kanten: &HashMap<Kante, u128>) -> Self {
        Self {
            fehler_bis: orte
                .iter()
                .circular_tuple_windows()
...
impl Fenster {
    fn fehler_auf_pfad(&self, kanten: &HashMap<Kante, u128>, pfad: &Pfad2) -> u8 {
        (0..=pfad.orte.len())
            .map(|i| (i % pfad.orte.len()) as u8)
            .map(|i| {
                let j = if (self.von..self.von + self.breite).contains(&i) {
                    self.zu + (i - self.von)
                } else {
                    let mut j = i;
                    if j >= self.zu {
                    ...
                    if j >= self.von + self.breite {
                        j -= self.breite;
                    }
                    j
                };
                pfad.orte[j as usize]
            })
            .tuple_windows()
            .map(|(von, uber, zu)| {
                kanten
                    .get(&Kante { von, zu: uber })
                    .filter(|nach| **nach & zu.s != 0)
                    .map_or(0, |_| 1)
            })
            .sum()
    }
...
mod fenster_tests {
    ...
    use crate::{Fenster, Kante, Ort, Pfad2};

    fn dummy_orte(i: usize) -> Vec<Ort> {
        (0..i)
            .map(|i| Ort {
                x: 0,
                y: 0,
    ...
    #[test]
    fn breite_groesser_2() {
        let orte = dummy_orte(7);
        let kanten = HashMap::from([
            (
                Kante {
                    von: orte[0],
                    zu: orte[1],
        ...
        let pfad = Pfad2::new(orte, &kanten);
        assert_eq!(
            Fenster {
                von: 2,
                zu: 4,
