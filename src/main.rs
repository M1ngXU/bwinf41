...
fn main() {
    ...
    assert!(
        command
            ...
            .wait()
            .expect("Cargo konnte nicht erfolgreich gelaufen werden.")
            .success(),
        "Cargo ist nicht erfolgreich beendet."
    );
    ...
    for aufgabe in ["a1", "a3", "a5"] {
        ...
        let mut bin = Path::new("bwinf41-1")
            .join(aufgabe)
            .join("AusfuehrbaresProgramm");
        std::fs::create_dir_all(&bin)
            .expect("Der Ordner `AusfuehrbaresProgramm` konnte nicht erstellt werden.");
        ...
        let size = exe
            .metadata()
            .expect("Metadaten konnten nicht gelesen werden.")
            .len();
        assert_eq!(
            size,
            std::fs::copy(exe, bin).expect("Die ausführbare Datei konnte nicht kopiert werden."),
            "Die ausführbare Datei konnte nicht vollständig kopiert werden."
        );
    ...
    zip_dir::zip_dir(
        Path::new("bwinf41-1"),
        File::create("bwinf41-1.zip").expect("Failed to open zip file."),
        None,
    )
